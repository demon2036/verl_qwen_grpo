...
"""
模块用途：将 DAPO-Math-17k 数据集补充为“多轮 + 工具调用”格式并保存为 parquet。  # 注释：模块用途
输入：DAPO-Math-17k 原始数据（本地或 HF Hub）与命令行参数。  # 注释：输入说明
输出：train.parquet（含 extra_info.tools_kwargs 等字段）。  # 注释：输出说明
关键依赖：datasets、verl.utils.hdfs_io、os。  # 注释：依赖说明
典型用法：  # 注释：用法标题
- python examples/data_preprocess/dapo_multiturn_w_tool.py --local_save_dir ~/data/retool_dapo  # 注释：最小命令
调用路径概览：  # 注释：调用链标题
...
if __name__ == "__main__":  # 注释：脚本入口
    ...
    parser.add_argument(  # 注释：添加保存目录参数
        "--local_save_dir", default="~/data/retool_dapo", help="The save directory for the preprocessed dataset."  # 注释：默认保存目录
    )  # 注释：参数定义结束

    args = parser.parse_args()  # 注释：解析参数
    local_dataset_path = args.local_dataset_path  # 注释：读取本地数据路径

    data_path = "BytedTsinghua-SIA/DAPO-Math-17k"  # 注释：默认数据集路径
    ...
    def make_map_fn(split):  # 注释：构造 map 处理函数
        ...
        调用路径依赖：  # 注释：调用路径说明标题
        - 所在位置：examples/data_preprocess/dapo_multiturn_w_tool.py::make_map_fn。  # 注释：函数位置
        ...
        def process_fn(example, idx):  # 注释：处理单条样本
            ...
            调用路径依赖：  # 注释：调用路径说明标题
            - 所在位置：examples/data_preprocess/dapo_multiturn_w_tool.py::process_fn。  # 注释：函数位置
