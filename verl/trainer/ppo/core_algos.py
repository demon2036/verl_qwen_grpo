...
"""
模块用途：实现 PPO/GRPO 等核心算法（优势估计、损失函数、KL 惩罚等）。（注释：模块级用途概述）
输入/输出：（注释：模块级 I/O 概览）
  - 输入：token-level 奖励、log_prob、mask、配置参数等。（注释：输入来源）
  - 输出：优势/回报/损失/指标等张量与字典。（注释：输出形态）
关键依赖：（注释：列出关键依赖）
  - torch / numpy。（注释：数值计算）
  - omegaconf.DictConfig。（注释：配置）
典型用法（最小示例）：（注释：最小用法）
  >>> adv_fn = get_adv_estimator_fn(AdvantageEstimator.GRPO)  # 获取估计器（示例）
  >>> adv, ret = adv_fn(token_level_rewards, response_mask, index=uid)  # 计算优势（示例）
调用路径概览：（注释：入口链路）
  - `RayPPOTrainer.fit` -> `compute_advantage` -> 本文件优势函数。（注释：调用链）
"""  # 注释：模块 docstring 结束
...
def register_policy_loss(name: str) -> Callable[[PolicyLossFn], PolicyLossFn]:
    """
    注册 policy loss 函数到全局注册表。（注释：函数用途）

    参数：（注释：参数说明）
      - name (str): 注册名称。（注释：输入含义）
    返回：（注释：返回值说明）
      - decorator (callable): 用于注册的装饰器。（注释：输出含义）
    副作用：（注释：副作用说明）
      - 修改 POLICY_LOSS_REGISTRY。（注释：全局状态）
    最小示例：（注释：最小示例）
      >>> @register_policy_loss("ppo_clip")\n... def loss_fn(...): ...  # 示例
    调用路径依赖：（注释：调用关系说明）
      - `compute_policy_loss_*` 在注册后可被 worker 调用。（注释：调用链）
    """

    def decorator(func: PolicyLossFn) -> PolicyLossFn:
        POLICY_LOSS_REGISTRY[name] = func  # 注释：写入注册表
        return func
...
def get_policy_loss_fn(name):
    """
    根据名称获取 policy loss 函数。（注释：函数用途）

    参数：（注释：参数说明）
      - name (str): loss 名称。（注释：输入含义）
    返回：（注释：返回值说明）
      - callable: 对应的 loss 函数。（注释：输出含义）
    异常/边界条件：（注释：异常说明）
      - 未注册名称会抛 ValueError。（注释：边界）
    最小示例：（注释：最小示例）
      >>> loss_fn = get_policy_loss_fn("ppo_clip")  # 示例
    """
    loss_name = name
...
class AdvantageEstimator(str, Enum):
    """
    优势估计器枚举，避免字符串拼写错误。（注释：类用途）

    说明：（注释：补充说明）
      - 可用字符串注册自定义估计器（register_adv_est）。（注释：扩展方式）
    """
...
def register_adv_est(name_or_enum: str | AdvantageEstimator) -> Any:
    """
    注册优势估计函数到 ADV_ESTIMATOR_REGISTRY。（注释：函数用途）

    参数：（注释：参数说明）
      - name_or_enum (str|AdvantageEstimator): 注册名或枚举值。（注释：输入含义）
    返回：（注释：返回值说明）
      - decorator: 装饰器，用于注册函数。（注释：输出含义）
    副作用：（注释：副作用说明）
      - 修改 ADV_ESTIMATOR_REGISTRY。（注释：全局状态）
    最小示例：（注释：最小示例）
      >>> @register_adv_est("my_est")\n... def my_fn(...): ...  # 示例
    """
...
def get_adv_estimator_fn(name_or_enum):
    """
    根据名称/枚举获取优势估计函数。（注释：函数用途）

    参数：（注释：参数说明）
      - name_or_enum (str|AdvantageEstimator): 注册名或枚举值。（注释：输入含义）
    返回：（注释：返回值说明）
      - callable: 优势估计函数。（注释：输出含义）
    异常/边界条件：（注释：异常说明）
      - 未注册名称会抛 ValueError。（注释：边界）
    """
    name = name_or_enum.value if isinstance(name_or_enum, Enum) else name_or_enum
...
class AdaptiveKLController:
    """
    自适应 KL 控制器（参见 1909.08593）。（注释：类用途）
    """
...
class FixedKLController:
    """固定 KL 控制器（系数不变）。（注释：类用途）"""

    def __init__(self, kl_coef):
        self.value = kl_coef

    def update(self, current_kl, n_steps):
        """固定 KL 更新为 no-op。（注释：方法用途）"""
        pass


def get_kl_controller(kl_ctrl):
    """
    根据配置创建 KL 控制器实例。（注释：函数用途）

    参数：（注释：参数说明）
      - kl_ctrl: 配置对象（含 type/kl_coef 等字段）。（注释：输入含义）
    返回：（注释：返回值说明）
      - FixedKLController 或 AdaptiveKLController。（注释：输出含义）
    异常/边界条件：（注释：异常说明）
      - type 未支持会抛 NotImplementedError；adaptive 的 horizon<=0 会断言失败。（注释：边界）
    """
...
def compute_gae_advantage_return(
    ...
    """
    计算 GAE 优势与回报。（注释：函数用途）

    参数：（注释：参数说明）
      - token_level_rewards (Tensor): 形状 (B,T) 的 token 奖励。（注释：输入含义）
      - values (Tensor): 形状 (B,T) 的价值估计。（注释：输入含义）
      - response_mask (Tensor): 形状 (B,T) 的有效 mask。（注释：输入含义）
      - gamma (float): 折扣因子。（注释：输入含义）
      - lam (float): GAE λ 参数。（注释：输入含义）
    返回：（注释：返回值说明）
      - advantages (Tensor): 形状 (B,T)。（注释：输出含义）
      - returns (Tensor): 形状 (B,T)。（注释：输出含义）
    最小示例：（注释：最小示例）
      >>> adv, ret = compute_gae_advantage_return(rewards, values, mask, 0.99, 0.95)  # 示例
    调用路径依赖：（注释：调用关系说明）
      - `RayPPOTrainer.compute_advantage` -> `compute_gae_advantage_return`。（注释：调用链）
    """
    with torch.no_grad():
        nextvalues = 0
        lastgaelam = 0
        ...
        returns = advantages + values  # 注释：回报 = 优势 + 价值
        advantages = verl_F.masked_whiten(advantages, response_mask)  # 注释：标准化优势
...
def compute_grpo_outcome_advantage(
    ...
    """
    GRPO outcome 优势：按 prompt 组内标准化奖励。（注释：函数用途）

    参数：（注释：参数说明）
      - token_level_rewards (Tensor): (B,T) token 奖励。（注释：输入含义）
      - response_mask (Tensor): (B,T) mask。（注释：输入含义）
      - index (np.ndarray): 分组索引（同一 prompt 的样本共享 id）。（注释：输入含义）
      - epsilon (float): 防止除零的小常数。（注释：输入含义）
      - norm_adv_by_std_in_grpo (bool): 是否按 std 归一化。（注释：输入含义）
      - config (AlgoConfig|None): 预留参数。（注释：输入含义）
    返回：（注释：返回值说明）
      - advantages (Tensor): (B,T)。（注释：输出含义）
      - returns (Tensor): (B,T)，此处等同于优势。（注释：输出含义）
    最小示例：（注释：最小示例）
      >>> adv, ret = compute_grpo_outcome_advantage(rewards, mask, index)  # 示例
    调用路径依赖：（注释：调用关系说明）
      - `compute_advantage` -> `compute_grpo_outcome_advantage`。（注释：调用链）
    """
    scores = token_level_rewards.sum(dim=-1)  # 注释：序列级 reward

    id2score = defaultdict(list)  # 注释：按 prompt 分组
    id2mean = {}
    id2std = {}
    ...
    with torch.no_grad():
        bsz = scores.shape[0]
        for i in range(bsz):
            id2score[index[i]].append(scores[i])  # 注释：收集同组样本
        for idx in id2score:
            if len(id2score[idx]) == 1:
                id2mean[idx] = torch.tensor(0.0)  # 注释：单样本组均值置 0
                id2std[idx] = torch.tensor(1.0)  # 注释：单样本组 std 置 1
            elif len(id2score[idx]) > 1:
                scores_tensor = torch.stack(id2score[idx])
                id2mean[idx] = torch.mean(scores_tensor)  # 注释：组均值
                id2std[idx] = torch.std(scores_tensor)  # 注释：组标准差
            else:
                raise ValueError(f"no score in prompt index: {idx}")  # 注释：异常保护
        for i in range(bsz):
            if norm_adv_by_std_in_grpo:
                scores[i] = (scores[i] - id2mean[index[i]]) / (id2std[index[i]] + epsilon)  # 注释：标准化
            else:
                scores[i] = scores[i] - id2mean[index[i]]  # 注释：仅去均值
        scores = scores.unsqueeze(-1) * response_mask  # 注释：扩展回 token 维并 mask

    return scores, scores  # 注释：GRPO 这里 returns=advantages
...
def compute_grpo_vectorized_outcome_advantage(
    ...
    """
    向量化 GRPO（仅 outcome）：按组标准化 reward 并广播到 token 维。（注释：函数用途）

    参数/返回含义与 compute_grpo_outcome_advantage 一致。（注释：简化说明）
    """
    with torch.no_grad():
...
def compute_grpo_passk_outcome_advantage(
    ...
    **kwargs,
    ...
    """
    GRPO Pass@k：每组仅最佳样本获得优势（r_max - r_second_max）。（注释：函数用途）

    说明：（注释：简要说明）
      - 参考 https://arxiv.org/abs/2503.19595。（注释：参考论文）
    """
...
def compute_rewards(token_level_scores, old_log_prob, ref_log_prob, kl_ratio):
    """
    计算带 KL 惩罚的 token-level 奖励。（注释：函数用途）

    参数：（注释：参数说明）
      - token_level_scores (Tensor): 原始 token 奖励。（注释：输入含义）
      - old_log_prob (Tensor): 当前策略 log_prob。（注释：输入含义）
      - ref_log_prob (Tensor): 参考策略 log_prob。（注释：输入含义）
      - kl_ratio (float): KL 系数。（注释：输入含义）
    返回：（注释：返回值说明）
      - Tensor: 加 KL 惩罚后的奖励。（注释：输出含义）
    """
    kl = old_log_prob - ref_log_prob  # 注释：KL 近似项
    return token_level_scores - kl * kl_ratio  # 注释：奖励减去 KL 惩罚
...
def agg_loss(
    ...
    """
    聚合 loss（确保在 FSDP/Megatron 下可比）。（注释：函数用途）

    参数：（注释：参数说明）
      - loss_mat (Tensor): (B,T) loss 矩阵。（注释：输入含义）
      - loss_mask (Tensor): (B,T) mask。（注释：输入含义）
      - loss_agg_mode (str): 聚合模式。（注释：输入含义）
      - dp_size / batch_num_tokens / global_batch_size / loss_scale_factor: 归一化参数。（注释：输入含义）
    返回：（注释：返回值说明）
      - Tensor: 标量 loss。（注释：输出含义）
    """
...
def kl_penalty(logprob: torch.FloatTensor, ref_logprob: torch.FloatTensor, kl_penalty) -> torch.FloatTensor:
    """计算 KL 惩罚项（支持多种近似与 straight-through）。 （注释：函数用途）"""
...
def compute_pf_ppo_reweight_data(
    ...
    """根据 token_level_scores 进行重加权（PF-PPO）。 （注释：函数用途）"""
...
def compute_policy_loss_bypass_mode(
    ...
    """Bypass 模式策略损失（支持 REINFORCE 与 PPO-clip）。  # 注释：中文概要
